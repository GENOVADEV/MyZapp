...
export function useConversations(filters?: ConversationFilters, page: number = 1, limit: number = 50) {
  ...
  const {
    ...
    queryFn: async (): Promise<ConversationsResponse> => {
      ...
      const response = await fetch(`/api/data/conversations?${params}`);
  ...
  const togglePinMutation = useMutation({
    mutationFn: async (conversationId: string) => {
      const response = await fetch(`/api/data/conversations/${conversationId}/pin`, {
        method: 'PATCH',
  ...
  const toggleArchiveMutation = useMutation({
    mutationFn: async (conversationId: string) => {
      const response = await fetch(`/api/data/conversations/${conversationId}/archive`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (conversationId: string) => {
      const response = await fetch(`/api/data/conversations/${conversationId}/read`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  const moveToFolderMutation = useMutation({
    mutationFn: async ({ conversationId, folderId }: { conversationId: string; folderId?: string }) => {
      const response = await fetch(`/api/data/conversations/${conversationId}/folder`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const searchConversations = useCallback(async (query: string): Promise<Conversation[]> => {
    const response = await fetch(`/api/data/conversations/search?q=${encodeURIComponent(query)}`);
  ...
  const getStats = useCallback(async () => {
    const response = await fetch('/api/data/conversations/stats');
...
export function useConversation(conversationId: string) {
  const {
    ...
    queryFn: async (): Promise<Conversation> => {
      const response = await fetch(`/api/data/conversations/${conversationId}`);
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/data/conversations/${conversationId}/messages?page=${page}&limit=${limit}`
      );
