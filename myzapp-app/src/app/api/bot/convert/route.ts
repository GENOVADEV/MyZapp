...
export async function POST(req: Request) {
  try {
    ...
    const { type, text, imageUrl } = await req.json();

    const botServerUrl = process.env.BOT_SERVER_URL || "https://myzapp-bot.onrender.com";
    const res = await fetch(`${botServerUrl}/api/bot/convert`, {
      method: "POST",
    ...
    if (!res.ok) {
      return NextResponse.json({ error: data.error || "Erreur de conversion" }, { status: res.status });
    }
